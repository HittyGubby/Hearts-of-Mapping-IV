...
function validateProvinceInState(provinces: (Province | undefined | null)[], states: (State | undefined | null)[], badStatesCount: number, warnings: WorldMapWarning[]) {
    ...
    for (let i = badStatesCount; i < states.length; i++) {
        ...
        if (!state || typeof state.id !== 'number' || !Array.isArray(state.provinces)) {
            warnings.push({
                source: [{ type: 'state', id: i }],
                text: localize('worldmap.warnings.invalidstate', 'State at index {0} is invalid.', i),
                relatedFiles: []
            });
            continue;
        }
        ...
        state.provinces.forEach(p => {
            if (p < 0 || p >= provinces.length) {
                warnings.push({
                    source: [{ type: 'state', id: state.id }],
                    text: localize('worldmap.warnings.invalidprovince', 'Invalid province index {0} in state {1}.', p, state.id),
                    relatedFiles: []
                });
                return;
            }
